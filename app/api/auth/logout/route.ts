...
export async function GET(request: Request) {
  ...
  // Redirect to Shopify's logout endpoint to terminate session there too
  const logoutUrl = `https://shopify.com/authentication/${shop_id}/logout?client_id=${client_id}&post_logout_redirect_uri=${encodeURIComponent(appUrl)}`;
